<x-app-layout title="Edit shipping profile">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit shipping profile') }}
        </h2>
        <div>
            <x-primary-button form="shipping_profile_form">Update</x-primary-button>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('dashboard.shippings.update', $shipping) }}" method="post" id="shipping_profile_form">
                @csrf
                @method('PUT')
                            ...
                            <div class="p-6 text-gray-900">
                                <div class="flex items-start gap-6">
                                    <div class="w-full">
                                        ...
                                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $shipping->name)" required autofocus />
                                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                                    </div>
                                    <div class="w-full" id="costField">
                                        <x-input-label for="cost" :value="__('Cost')" />
                                        <x-text-input id="cost" class="block mt-1 w-full" type="text" name="cost" step="0.01" :value="old('cost', $shipping->cost)" />
                                ...
                                <div class="flex items-center gap-6 mt-4">
                                    <div class="w-full" id="shippingMaxDurationField">
                                        ...
                                        <x-text-input id="max_delivery_estimate" class="block mt-1 w-full" type="text" name="max_delivery_estimate" :value="old('max_delivery_estimate', $shipping->max_delivery_estimate)" required />
                                    ...
                                    <div class="w-full" id="shippingMinDurationField">
                                        ...
                                        <x-text-input id="min_delivery_estimate" class="block mt-1 w-full" type="text" name="min_delivery_estimate" :value="old('min_delivery_estimate', $shipping->min_delivery_estimate)" required />
                                        <x-input-error :messages="$errors->get('min_delivery_estimate')" class="mt-2" />
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <x-input-label for="description" :value="__('Description (optional)')" />
                                    <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $shipping->description) }}</textarea>
                                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                                </div>
                                <div class="mt-4 flex items-start gap-5">
                                    <div class="relative flex gap-x-3">
                                        <div class="flex h-6 items-center">
                                            <input id="is_free" name="is_free" type="checkbox" {{ old('is_free', $shipping->is_free) ? 'checked' : '' }} value="1" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" />
                                        </div>
                                    ...
                                    <div class="relative flex gap-x-3">
                                        <div class="flex h-6 items-center">
                                            <input id="is_active" name="is_active" type="checkbox" {{ old('is_active', $shipping->is_active) ? 'checked' : '' }} value="1" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" />
                                        </div>
                                        <div class="text-sm leading-6">
                                ...
                                <div class="mt-6">
                                    ...
                                    <div id="conditions-container" class="mt-2">
                                        @foreach($shipping->conditions as $index => $condition)
                                            <div class="condition-row mt-2 flex items-center space-x-2">
                                                <select name="conditions[{{ $index }}][type]" class="block rounded-md border-0 py-1.5 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                                    <option value="min_total" {{ $condition['type'] == 'min_total' ? 'selected' : '' }}>Minimum Total</option>
                                                    <option value="max_total" {{ $condition['type'] == 'max_total' ? 'selected' : '' }}>Maximum Total</option>
                                                    <option value="min_items" {{ $condition['type'] == 'min_items' ? 'selected' : '' }}>Minimum Items</option>
                                                    <option value="max_items" {{ $condition['type'] == 'max_items' ? 'selected' : '' }}>Maximum Items</option>
                                                </select>
                                                <x-text-input type="number" name="conditions[{{ $index }}][value]" value="{{ $condition['value'] }}" placeholder="Value" required />
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let conditionCount = {{ count($shipping->conditions) }};
